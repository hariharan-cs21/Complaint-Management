    ...
    if (Category && summary && description) {
      ...
      try {
        await addDoc(queryCollection, {
          ...
          summary,
          description,
          queryPerson: { name: auth.currentUser.displayName, id: auth.currentUser.uid }
        });
        alert("Uploaded")
        setCategory("");
        setSummary("");
        ...
        <div className="flex items-center mb-6">
          ...
          <div className="w-11/12">
            <input
              ...
              className="bg-white appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-black leading-tight focus:outline-none focus:bg-white rounded-md"
              id="inline-password"
            />          </div>
        </div>
